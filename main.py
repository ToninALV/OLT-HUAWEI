...
def olt_date(): #Coletar hora Atual da OLT#
    telnet.write(b'display time\n\n')
    telnet.write(b'teste\n')
    time_olt = telnet.read_until(b'teste', timeout=5).decode('utf8')
    lines = time_olt.splitlines()
    
    time_olt = ''.join(lines[-3])
    
    time_olt = time_olt[2:12]
    olt_date = datetime.strptime(time_olt, '%Y-%m-%d').date()
...
#telnet.write(b'undo mmi-mode original-output')
atual_date = olt_date()
i = -1
while i < len(list):  
    i += 1
        ...
        with open(path2, 'r') as arquivo:         
            for line in arquivo:
                if ("SN") in line and ("SN-auth") not in line:
                    ...
                    if result[4] == '-':
                        pass
                        #print(f"Onu a ser deletada de index {index} frame {frame} slot {slot} port {port_pon} onu {onu_id} pois a mesma dessincronizou com a OLT.")
                        ...
                        #diferenca = olt_date() - last_down_time 
